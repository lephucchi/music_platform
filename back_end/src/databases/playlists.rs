...
#[async_trait]
impl PlaylistExt for DBClients {
    async fn create_playlist(
        &self,
        user_id: Uuid,
        title: String,
    ...
    async fn get_last_track_order (
        ...
        Ok((result.track_order.unwrap_or(0)) as i32)
    }
    ...
    async fn get_user_playlists(
        ...
        let query = sqlx::query_as!(
            r#"
            SELECT p.id, p.title, p.thumbnail_path, COALESCE(MAX(pt.track_order), 0) AS max_track_order
            FROM playlists p
            WHERE p.user_id = $1
    ...
    async fn get_playlist_tracks(
        ...
        let query = sqlx::query_as!(
            ...
            r#"
            SELECT 
                t.id, t.title, t.artist, t.duration, t.flie_name, t.upload_status, t.thumbnail_name, 
                ...
                CASE WHEN t.uer_id = $2 THEN true ELSE false END as is_created_by_user
